...
var EventbriteStrategy = require('passport-eventbrite-oauth/oauth2');


vows.describe('EventbriteStrategy').addBatch({
  
  'strategy': {
    topic: function() {
      return new EventbriteStrategy({
        clientID: 'ABC123',
        clientSecret: 'secret'
      },
      function() {});
    ...
    'should be named eventbrite': function (strategy) {
      assert.equal(strategy.name, 'eventbrite');
    },
  },
  ...
  'strategy when loading user profile': {
    ...
    'when told to load user profile': {
      ...
      'should load profile' : function(err, profile) {
        assert.equal(profile.provider, 'eventbrite');
